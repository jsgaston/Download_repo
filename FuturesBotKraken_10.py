...
MAX_SL_PCT          = 6.0         # SL máximo como % del precio (rechaza riesgo excesivo)
...
MAX_ORDER_AGE_HOURS = 12          # Horas máximas de vida de una posición

# ── Horario de trading ───────────────────────────────────────
NO_TRADE_START_HOUR = 23          # Hora inicio de pausa nocturna (formato 24h)
NO_TRADE_END_HOUR   = 6           # Hora fin de pausa nocturna (formato 24h)
        ...
        try:
            ...
            if df is not None and len(df) >= 20:
                ...
                if side == "buy":
                    ...
                    if (price - recent_low) < sl_distance_abs * 0.8:
                        return False, (f"Precio demasiado cerca del mínimo reciente 1H "
                                       f"(mín=${recent_low:.4f}, gap=${price-recent_low:.4f} < "
                                       f"0.8x SL=${sl_distance_abs:.4f}) — SL quedaría bajo soporte reciente")
                else:
                    recent_high = recent["High"].max()
                    if (recent_high - price) < sl_distance_abs * 0.8:
                        return False, (f"Precio demasiado cerca del máximo reciente 1H "
                                       f"(máx=${recent_high:.4f}, gap=${recent_high-price:.4f} < "
                                       f"0.8x SL=${sl_distance_abs:.4f}) — SL quedaría sobre resistencia reciente")
        except Exception:
